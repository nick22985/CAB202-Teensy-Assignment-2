...
void respawn_snake(int seed) {
    ...
    sprite_init(&snake[0], x, y, 3, 3, snake_bitmap);
    sprite_init(&snake[1], snake[0].x, y-4, 3, 3, snake_bitmap);
    snake[0].dx = 0;
    snake[0].dy = 0;
}
...
int draw_wall(int x_start, int y_start, int length, int height) {
    ...
    draw_line(wall_left, wall_top, wall_right, wall_bottom, FG_COLOUR);
...
int main() {
    ...
    draw_string(5,15,"Eliot Whalan", FG_COLOUR);
    draw_string(5,25,"n9446800", FG_COLOUR);
    ...
    sprite_init(&food, 42, 12, 2, 2, food_bitmap);
    uint16_t adc_result;
    ...
    while(lives != 0){
        ...
        if (sprites_collided()) {
            ...
            sprite_init(&snake[length], snake[length-2].x, snake[length-2].y, 3, 3, snake_bitmap);
    ...
    ////array is too big for the screen size, so we break it into 3 parts
    draw_string(5,15,"Game Over", FG_COLOUR);
